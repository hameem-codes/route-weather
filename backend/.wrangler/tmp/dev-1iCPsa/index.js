...
// .wrangler/tmp/bundle-7NPR6h/checked-fetch.js
...
// .wrangler/tmp/bundle-7NPR6h/strip-cf-connecting-ip-header.js
...
app.get("/api/route", async (c) => {
  ...
  try {
    const originRes = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(origin)}&format=json&limit=1`, {
      headers: { "User-Agent": "RouteWeatherApp/1.0" }
    });
    const originData = await originRes.json();
    if (!originData.length)
      return c.json({ error: "Origin not found" }, 404);
...
// .wrangler/tmp/bundle-7NPR6h/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-7NPR6h/middleware-loader.entry.ts
