...
// .wrangler/tmp/bundle-bzGaOc/checked-fetch.js
...
// .wrangler/tmp/bundle-bzGaOc/strip-cf-connecting-ip-header.js
...
app.get("/api/route", async (c) => {
  ...
  if (oLat < -90 || oLat > 90 || dLat < -90 || dLat > 90)
    return sendError(c, "VALIDATION_ERROR", "Latitude must be between -90 and 90");
  if (oLng < -180 || oLng > 180 || dLng < -180 || dLng > 180)
    return sendError(c, "VALIDATION_ERROR", "Longitude must be between -180 and 180");
  ...
  try {
    ...
    if (!osrmRes.ok)
      throw new Error("UPSTREAM_SERVICE_FAILED");
    ...
    if (err.name === "AbortError")
      return sendError(c, "UPSTREAM_TIMEOUT", "Request to routing service timed out", 504);
    if (err.message === "RATE_LIMIT_EXCEEDED")
      return sendError(c, "RATE_LIMIT_EXCEEDED", "Routing service rate limit exceeded", 429);
...
app.get("/api/geocode", async (c) => {
  ...
  try {
    ...
    if (err.name === "AbortError")
      return sendError(c, "UPSTREAM_TIMEOUT", "Request to geocoding service timed out", 504);
    if (err.message === "RATE_LIMIT_EXCEEDED")
      return sendError(c, "RATE_LIMIT_EXCEEDED", "Geocoding service rate limit exceeded", 429);
...
app.post("/api/weather/route", async (c) => {
  try {
    ...
    if (err.name === "AbortError")
      return sendError(c, "UPSTREAM_TIMEOUT", "Request to weather service timed out", 504);
    if (err.message === "RATE_LIMIT_EXCEEDED")
      return sendError(c, "RATE_LIMIT_EXCEEDED", "Weather service rate limit exceeded", 429);
...
app.post("/api/route-weather", async (c) => {
  try {
    ...
    const geocode = /* @__PURE__ */ __name(async (query) => {
      ...
      const data = await res.json();
      if (!data || data.length === 0)
        throw new Error(`LOCATION_NOT_FOUND: ${query}`);
      const result = {
    ...
    if (err.name === "AbortError")
      return sendError(c, "UPSTREAM_TIMEOUT", "Request to an upstream service timed out", 504);
    if (err.message === "RATE_LIMIT_EXCEEDED")
      return sendError(c, "RATE_LIMIT_EXCEEDED", "An upstream service rate limit was exceeded", 429);
    if (err.message.startsWith("LOCATION_NOT_FOUND"))
      return sendError(c, "LOCATION_NOT_FOUND", err.message.replace("LOCATION_NOT_FOUND: ", "Could not find location: "), 404);
    if (err.message === "ROUTE_NOT_FOUND")
      return sendError(c, "ROUTE_NOT_FOUND", "Could not find a valid driving route", 404);
...
// .wrangler/tmp/bundle-bzGaOc/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-bzGaOc/middleware-loader.entry.ts
